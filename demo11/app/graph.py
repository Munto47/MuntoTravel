"""
graph.py —— demo11：两阶段 Multi-Agent 图 + 地图坐标导出

架构对比（与 demo10 的差异）：
  demo10：route_node 计算路线后直接返回 trip_plan
  demo11：route_node 额外调用 get_trip_coords() 将本次行程坐标打包到 coord_map，
          前端用此坐标在高德 JSAPI 2.0 上绘制 Marker + Polyline 动画

新增数据流：
  route_node 结束后：
    all_locations = trip_plan.days[i].locations（所有天合并）
    coord_map = get_trip_coords(locations_by_day, city)   ← 从 _coord_cache 提取
    return {"trip_plan": ..., "coord_map": coord_map}

  run_graph 返回值新增 coord_map（第6个返回值）

图结构不变（与 demo09/10 相同）：
  START → dispatch_agents (Fan-out)
  [weather/poi/transport 并行] → Fan-in
  → planner_node → route_node → END
"""
...
from .route_client import get_trip_coords, plan_day_routes
...
class TravelState(TypedDict):
    """
    demo11 扩展：新增 coord_map 字段（行程地点坐标，route_node 填充，前端地图用）。

    demo10 字段保留：
      profile_note: 来自问卷的用户画像文本
    ...
    # demo11 新增：行程地点坐标（route_node 写入，main.py 读取后传给前端）
    coord_map: Optional[dict]
...
async def route_node(state: TravelState) -> dict:
    ...
    # demo11 新增：从缓存提取行程所有地点坐标，供前端高德 JSAPI 2.0 渲染
    locations_by_day = [d.get("locations", []) for d in trip_plan.get("days", [])]
    coord_map = get_trip_coords(locations_by_day, city)

    ms = int((time.perf_counter() - t0) * 1000)
    source = "高德地图实时" if api_key else "距离估算"
    logger.info("[Route] 全部完成 (%dms) · %d段路线 · %s · %d个坐标",
                ms, total_segs, source, len(coord_map))

    route_log = {
        "agent": "route", "label": "路线专员", "icon": "🗺️",
        "status": "ok" if api_key else "warn",
        "duration_ms": ms,
        "detail": f"{total_segs}段路线 · {source} · {len(coord_map)}个坐标",
        "source": source,
    }
    return {"trip_plan": trip_plan, "coord_map": coord_map, "agent_logs": [route_log]}
...
def get_graph():
    ...
    if _graph is None:
        _graph = build_graph()
        logger.info("[Graph] demo11 两阶段图编译完成（含坐标导出）")
    return _graph
...
async def run_graph(
    request: PlanRequest,
) -> tuple[Optional[TripPlan], list[dict], Optional[dict], list[dict], str, dict]:
    """
    返回值（6项，demo11 新增 coord_map）：
    ...
    coord_map  = final_state.get("coord_map") or {}
    return plan, logs, final_state.get("transport_result"), catalog, smap, coord_map
