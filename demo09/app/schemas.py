"""
schemas.py —— demo09 数据模型

demo08 → demo09 新增：
  RouteSegment   相邻地点间的路线段（出行方式 + 耗时 + 距离）
  DayPlan.locations      LLM 输出的有序地点列表，供 route_node 使用
  DayPlan.route_segments route_node 填充的路线段列表

架构说明（两阶段图）：
  Phase 1 (并行)：weather_agent / poi_agent / transport_agent
  Phase 2 (顺序)：planner_node → route_node → END

  新 LangGraph 知识点：
    - planner_node 先生成带 locations 列表的行程草稿
    - route_node 读取 trip_plan.days[i].locations，逐段调用高德路线 API
    - route_node 将 route_segments 注入 trip_plan 后回写 state
    - 这是"LLM 先规划，工具后补充细节"的两阶段模式
"""
