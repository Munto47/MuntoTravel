...
demo03 vs demo04 的关键差异：
  demo03：手写 TOOL_DEFINITIONS (JSON Schema) + execute_tool() 函数
  demo04：用 @tool 装饰器，LangChain 自动生成 JSON Schema，ToolNode 自动执行

@tool 装饰器做了什么：
  1. 从函数签名和 docstring 自动生成 OpenAI Function Calling 所需的 JSON Schema
  2. 把函数包装成 LangChain BaseTool 对象，兼容所有 LangChain/LangGraph 组件
  3. Literal 类型注解会自动转成 enum 约束，无需手写
"""
...
# @tool 装饰器会读取：
#   - 函数名         → tool name
#   - docstring      → tool description（LLM 靠这个决定何时调用）
#   - 参数类型注解    → JSON Schema（Literal 自动变 enum）
