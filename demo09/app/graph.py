"""
graph.py —— demo09：两阶段 Multi-Agent 图
...
class TravelState(TypedDict):
    """
    demo09 扩展：新增 hotel 字段（路线规划起点）。

    pre-step 新增：
      poi_coords: poi_agent 搜索结果的坐标字典（name → "lng,lat"）
        由 poi_agent 写入，route_node 读取并预加载到 route_client 缓存
        → 同一地点无需重复 geocode，减少 AMAP API 请求数
    """
    city: str
    origin: str
...
5. 根据预算等级调整档次：low=平价，medium=均衡，high=品质优先
6. 餐厅推荐要有具体名称，尽量使用专家提供的数据
"""
...
async def route_node(state: TravelState) -> dict:
    ...
    for day in trip_plan.get("days", []):
        ...
        if len(locs) >= 2:
            ...
            segs: list[RouteSegment] = await plan_day_routes(locs, city, api_key, poi_coords)
...
def get_graph():
    ...
    if _graph is None:
        _graph = build_graph()
        logger.info("[Graph] demo09 两阶段图编译完成")
    return _graph
...
async def run_graph(
    request: PlanRequest,
) -> tuple[Optional[TripPlan], list[dict], Optional[dict]]:
    t0 = time.perf_counter()
