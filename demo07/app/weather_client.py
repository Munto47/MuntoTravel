    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            ...
            if geo_data.get("code") != "200" or not geo_data.get("location"):
                logger.warning("[QWeather !!] 城市搜索失败 code=%s city=%s", geo_data.get("code"), city)
                return None

            location_id = geo_data["location"][0]["id"]
            city_name   = geo_data["location"][0]["name"]
            logger.debug("[QWeather] Geo: %s → id=%s name=%s", city, location_id, city_name)
            ...
            if forecast_data.get("code") != "200":
                logger.warning("[QWeather !!] 预报查询失败 code=%s", forecast_data.get("code"))
                return None
        ...
        logger.warning("[QWeather !!] 请求异常：%s", e)
        return None
...
async def _get_openmeteo(city: str, days: int) -> WeatherInfo | None:
    ...
    if not coords:
        logger.warning("[OpenMeteo !!] %s 不在坐标缓存中，跳过（可在 _CITY_COORDS 中补充）", city)
        return None

    lat, lon = coords
    logger.debug("[OpenMeteo] %s → 坐标 (%.4f, %.4f)", city, lat, lon)
    params = {
    ...
    for attempt in range(2):
        ...
        try:
            ...
            logger.debug("[OpenMeteo OK] %s %d天 (%dms)", city, len(day_list), ms)
            return WeatherInfo(city=city, days=day_list, source="Open-Meteo")

        except Exception as e:
            if attempt == 0:
                logger.warning("[OpenMeteo] 第1次失败，1s 后重试：%s", e)
                await asyncio.sleep(1)
            else:
                logger.warning("[OpenMeteo !!] 最终失败：%s", e)
    return None
...
async def get_weather(city: str, days: int) -> WeatherInfo | None:
    ...
    """
    t0 = time.perf_counter()
    logger.info("[Weather] 请求：%s %d天", city, days)
    ...
    if qweather_key and qweather_host:
        logger.debug("[Weather] 使用和风天气（host=%s）", qweather_host)
        result = await _get_qweather(city, days, qweather_key, qweather_host)
        if result:
            elapsed = time.perf_counter() - t0
            logger.info("[Weather OK] 和风天气成功：%s %d天 (%.2fs)", city, len(result.days), elapsed)
            return result
        logger.warning("[Weather] 和风天气失败，降级到 Open-Meteo")
    else:
        logger.warning("[Weather] 未配置 QWEATHER_API_KEY/HOST，使用 Open-Meteo 备源")

    # 备源：Open-Meteo + 坐标缓存
    result = await _get_openmeteo(city, days)
    elapsed = time.perf_counter() - t0
    if result:
        logger.info("[Weather OK] Open-Meteo 成功：%s %d天 (%.2fs)", city, len(result.days), elapsed)
    else:
        logger.warning("[Weather !!] 两个来源均失败，%s 天气数据不可用 (%.2fs)", city, elapsed)
    return result
