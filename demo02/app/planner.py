...
async def create_trip_plan(request: TripRequest) -> TripPlan:
    # 第一步：并发获取天气（超时 8s 不阻塞主流程，失败返回 None）
    weather = await get_weather(request.city, request.travel_days)

    if weather:
        print(f"[Weather] 成功获取 {request.city} 天气：{len(weather.days)} 天")
    else:
        print(f"[Weather] 未能获取 {request.city} 天气，将不注入天气上下文")

    # 第二步：携带天气上下文调用 LLM
    try:
        raw_plan = await generate_trip_plan_json(request, weather)
        plan = TripPlan(**raw_plan)

        # 第三步：把天气摘要写入 plan，供前端直接展示
        if weather:
